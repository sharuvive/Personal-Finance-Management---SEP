...
//Declare UI elements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        helper = new DataBaseHelper(UpdateExpenses.this);
    ...
    private void fetchData4() {
        ...
        Cursor c = db.query(DataBaseHelper.TABLE3, null, DataBaseHelper.ID1 + "='" + value_in_tv + "'", null, null, null, null); //get the items from the database for the particular id
        if (c.moveToFirst()) {
    ...
    public void onClick(View v) {
        ...
        value.put(DataBaseHelper.EX_YEAR, SELECTED_DATE.split("-")[0]);  //Split the date to get the year
        value.put(DataBaseHelper.EX_MONTH, SELECTED_DATE.split("-")[1]);  //split the date to get month
        if (isValidnum2(Rs) && isValidWord(desc)) {
            db = helper.getWritableDatabase();
            db.update(DataBaseHelper.TABLE3, value, " " + DataBaseHelper.ID1 + "='" + value_in_tv + "'", null); //update the values to the database
            db.close();
